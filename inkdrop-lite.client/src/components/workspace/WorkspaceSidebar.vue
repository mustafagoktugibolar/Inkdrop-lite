...
const collapsedNotebooks = ref(new Set<string>())
const notebookRows = computed(() => buildNotebookRows(props.notebooks, collapsedNotebooks.value))
// The icon rail shows every notebook, in tree order, without the expand/collapse state.
const allNotebookRows = computed(() => buildNotebookRows(props.notebooks))

function toggleCollapsed(id: string) {
  const next = new Set(collapsedNotebooks.value)
  if (!next.delete(id)) next.add(id)
  collapsedNotebooks.value = next
}

function noteCount(kind: 'notebook' | 'tag' | 'status', id: string | NoteStatus) {
  if (kind === 'notebook') {
    const ids = new Set(notebookAndDescendantIds(props.notebooks, id as string))
    return props.notes.filter((note) => ids.has(note.notebookId)).length
  }
...
<template>
  <aside :class="cn('min-h-0 flex-col border-r bg-sidebar text-sidebar-foreground lg:flex', mobilePane === 'library' ? 'flex' : 'hidden')">
    <div v-if="collapsed" class="hidden min-h-0 flex-1 flex-col items-center lg:flex">
      <div class="flex h-12 w-full shrink-0 items-center justify-center border-b"><Button variant="ghost" size="icon-sm" aria-label="Expand sidebar" title="Expand sidebar" @click="emit('expand')"><HugeiconsIcon :icon="SidebarRightIcon" /></Button></div>
      <ScrollArea class="min-h-0 w-full flex-1">
        <nav class="flex flex-col items-center gap-1 py-2">
          <Button :variant="activeFilter.kind === 'all' ? 'secondary' : 'ghost'" size="icon-sm" aria-label="All Notes" title="All Notes" @click="emit('filter', { kind: 'all', label: 'All Notes' })"><HugeiconsIcon :icon="File01Icon" /></Button>
          <Button variant="ghost" size="icon-sm" aria-label="New note" title="New note" :disabled="busy" @click="emit('create')"><HugeiconsIcon :icon="NoteAddIcon" /></Button>
          <Separator class="my-1 w-6" />
          <Button v-for="row in allNotebookRows" :key="row.book.id" :variant="activeFilter.kind === 'notebook' && activeFilter.id === row.book.id ? 'secondary' : 'ghost'" size="icon-sm" :aria-label="row.book.name" :title="row.book.name" @click="emit('filter', { kind: 'notebook', id: row.book.id, label: row.book.name })"><NotebookIcon :notebook="row.book" /></Button>
          <Separator v-if="notebooks.length" class="my-1 w-6" />
          <Button v-for="status in noteStatuses" :key="status.value" :variant="activeFilter.kind === 'status' && activeFilter.id === status.value ? 'secondary' : 'ghost'" size="icon-sm" :aria-label="status.label" :title="status.label" @click="emit('filter', { kind: 'status', id: status.value, label: status.label })"><HugeiconsIcon :icon="status.icon" /></Button>
          <Separator v-if="tags.length" class="my-1 w-6" />
          <Button v-for="tag in tags" :key="tag.id" :variant="activeFilter.kind === 'tag' && activeFilter.id === tag.id ? 'secondary' : 'ghost'" size="icon-sm" :aria-label="tag.name" :title="tag.name" @click="emit('filter', { kind: 'tag', id: tag.id, label: tag.name })"><HugeiconsIcon :icon="Tag01Icon" /></Button>
        </nav>
      </ScrollArea>
    ...
    <div :class="cn('flex min-h-0 flex-1 flex-col', collapsed && 'lg:hidden')">
      <div class="hidden h-12 items-center gap-2 border-b px-4 lg:flex">
        ...
        <b class="flex-1 text-xs">Inkdrop Lite</b>
        <Button variant="ghost" size="icon-sm" aria-label="Collapse sidebar" title="Collapse sidebar" @click="emit('collapse')"><HugeiconsIcon :icon="SidebarLeftIcon" /></Button>
      </div>
      <ScrollArea class="flex-1">
        <nav class="flex flex-col gap-1 p-2">
          ...
          <div v-for="row in notebookRows" :key="row.book.id" class="flex items-center" :style="{ paddingLeft: `${row.depth * 0.75}rem` }">
            <Button v-if="row.hasChildren" variant="ghost" size="icon-xs" :aria-label="collapsedNotebooks.has(row.book.id) ? 'Expand notebook' : 'Collapse notebook'" :aria-expanded="!collapsedNotebooks.has(row.book.id)" @click="toggleCollapsed(row.book.id)"><HugeiconsIcon :icon="collapsedNotebooks.has(row.book.id) ? ArrowRight01Icon : ArrowDown01Icon" /></Button>
            <span v-else class="size-6 shrink-0" />
            <Button :variant="activeFilter.kind === 'notebook' && activeFilter.id === row.book.id ? 'secondary' : 'ghost'" class="min-w-0 flex-1 justify-start" @click="emit('filter', { kind: 'notebook', id: row.book.id, label: row.book.name })"><NotebookIcon :notebook="row.book" /><span class="flex-1 truncate text-left">{{ row.book.name }}</span><span class="text-[10px] text-muted-foreground">{{ noteCount('notebook', row.book.id) }}</span></Button>
          </div>
